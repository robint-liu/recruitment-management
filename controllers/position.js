...
module.exports = {
	...
	getPositionList(req, res) {
		...
		positionModel.getPosition({}, (result) => {
			if (result && result !== "error") {//为假
				totalPage = (Math.ceil(result.length / size))>=1?(Math.ceil(result.length / size)):1;//始终将totalPage最小值设为1
...
//======================position==========================================================
	addEngineer(req, res) {
		...
		const filename=req.file?req.file.filename:"";
		positionModel.addEngineer(name, sex, age, position,salary,address,filename,(err) => {
			res.json({
				ret: true,
				data: {
					inserted: !err
	...
	getEngineerList(req, res) {
		...
		positionModel.getEngineer({}, (result) => {
			if (result && result !== "error") {//为假
				totalPage = (Math.ceil(result.length / size))>=1?(Math.ceil(result.length / size)):1;
	...
	removeEngineer(req,res){
		positionModel.removeEngineerItemById(req.query.file,req.query.id,(err)=>{
			res.json({
				ret:true,
				data:{
					delete:!err
	...
	updateEngineer(req,res){
		...
		const params = {//当对象的键和值相等时，可忽略值。id不需要更新
			name,age,sex,position,salary,address
		}
		if (req.file && req.file.filename) {
			params.filename = req.file.filename
		}
	...
	EngGetposition(req,res){
		...
		positionModel.getPosition({salary:salary}, (result) => {
			//console.log(result);
			if (result && result !== "error"&&result.length!==0) {//为假
			res.json({//将json返回给前端
					ret: true,
					data: {
						list: result,
