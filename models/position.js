...
module.exports = {
  ...
  removeItemById: function (file, id, cb) {
    if (file !== "default.png") {
      fs.unlink("../public/position_logo/" + file, (err) => {
        console.log(err);
      });
    }
    Position.findByIdAndRemove(id, (err) => {
      //findByIdAndRemove封装好的方法
      cb(err);
    });
  },
  ...
  updatePositionById: function (id, imgSrc, params, cb) {
    if (imgSrc !== "/position_logo/default.png" && params.filename) {
  ...
  removeEngineerItemById: function (file, id, cb) {
    if (file !== "default.png") {
      fs.unlink(
        "../public/engineer_logo/" + file,
        (err) => {
          console.log(err);
        }
      );
    }
    Engineer.findByIdAndRemove(id, (err) => {
      //findByIdAndRemove封装好的方法
      cb(err);
    });
  },
  updateEngineerById: function (id, imgSrc, params, cb) {
    if (imgSrc !== "/engineer_logo/default.png" && params.filename) {
      fs.unlink("..public" + imgSrc, (err) => {
        console.log(err);
      });
    }
    Engineer.findByIdAndUpdate(id, params)
      .then((result) => {
        cb(result);
      })
      .catch(() => {
