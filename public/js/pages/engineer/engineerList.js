	...
	createItems:function(list){
		...
		if (list!="error") {
				...
				str+=`<tr>
							...
							<td style="text-align:center;cursor:pointer;border-radius:5px;" class="js-Nowsalary btn-default">${item.salary}</td>
							<td style="text-align:center">${item.address}</td>
							<td style="text-align:center"><img class="img_engineer" style="width:30px;height:30px;" src="/engineer_logo/${this.file}"/></td>
							<td style="text-align:center">
								<span class="js-update btn-default hover" style="box-shadow: 1px 1px 1px 1px rgba(0,0,0,.5);width:45%;background:orange;height: 30px;line-height: 30px;text-align: center;cursor:pointer;display:inline-block;color:#fff" data-id="${item._id}">修改</span>
								<span class="js-delete btn-default hover" style="box-shadow: 1px 1px 1px 1px rgba(0,0,0,.5);width:45%;background:red;height: 30px;line-height: 30px;text-align: center;cursor:pointer;display:inline-block;color:#fff" data-id="${item._id}" style="margin-left:20px">删除</span>
							</td>
						</tr>`;
				}
